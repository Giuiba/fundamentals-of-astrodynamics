...
int main()
{
	...
	double dat, dut1;	// ICRF osculating (in STK scenario HPOP objects)
	...
	iau06data iau06arr;
	EopSpw::iau06in(Nut80Loc, iau06arr);
...
//		strcpy_s(fname, "../GEOSatSRPHPOP11.e");
	...
	case 't': processtle(batchmode, proptype, derivType, cdam, cram, iau80arr, iau06arr, eoparr, jdeopstart, jdeopstartf, spwarr, jdspwstart, jdspwstartf,
		jpldearr, jplopt, jdjpldestart, jdepoch, jdepochf, interp, firstob, obsskip, obsread, obsarr, xnom);
		break;
	}
}  // main
...
#endif
				currobsrec.mon = MathTimeLib::getIntMonth(monstr);
	...
	while ((feof(infile) == 0) && (ii < obsread))   // read more in for comparison later
	{
		...
		if (strcmp(strr, "E") != 0)
		{
			...
			if ((strcmp(coords, "J2000") == 0) || (strcmp(coords, "MeanOfEpoch") == 0))
			{
				...
				AstroLib::eci_mod(rpos, vpos, apos, MathTimeLib::eTo, reci, veci, aeci, opt, ttt, jdut1 + jdut1f);
			}
			else
				if (strcmp(coords, "Fixed") == 0)
				{
					...
					MathTimeLib::convtime(year, mon, day, hr, min, sec, timezone, dut1, dat,
						ut1, tut1, jdut1, jdut1f, utc, tai, tt, ttt, jdtt, jdftt, tcg, tdb, ttdb, jdtdb, jdtdbf, tcb);
					AstroLib::eci_ecef(rpos, vpos, apos, MathTimeLib::eTo, reci, veci, aeci, opt, iau80rec, iau06rec, jdtt, jdftt, 
...
)
{
	...
	else
	{
		...
		for (ktr = 1; ktr < 10; ktr++)     // 10
		{
			...
			AstroLib::eci_ecef(r1, v1, aitrf, MathTimeLib::eTo, r1teme, v1teme, ateme, e80,
			...
			AstroLib::eci_ecef(r2, v2, aitrf, MathTimeLib::eTo, r2teme, v2teme, ateme, e80,
				iau80rec, iau06rec, jdtt, jdftt, jdut1 + jdut1f, jdxysstart, lod, xp, yp, ddpsi, ddeps, ddx, ddy, trans);
			...
			AstroLib::eci_ecef(r3, v3, aitrf, MathTimeLib::eTo, r3teme, v3teme, ateme, e80,
				iau80rec, iau06rec, jdtt, jdftt, jdut1 + jdut1f, jdxysstart, lod, xp, yp, ddpsi, ddeps, ddx, ddy, trans);
	...
	else
	{
		...
		while (feof(infile) == 0)
		{
			...
			if (feof(infile) == 0)
			{
				...
				fprintf(outfile, "%5s xx \n", satrec.satnum);
				printf(" %5s ", satrec.satnum);
			...
			*/
			...
			magdr = MathTimeLib::mag(dr) * 1000.0;
			fprintf(outfile, " difference %11lf  %11lf  %11lf %11lf \n", dr[0], dr[1], dr[2], MathTimeLib::mag(dr));
			fprintf(outfile1, " %8s  difference %11lf  %11lf  %11lf km %12lf m \n\n", satrec.satnum, dr[0], dr[1], dr[2], 1000.0 * MathTimeLib::mag(dr));
			...
			if (1000.0 * MathTimeLib::mag(dr) < 0.0001)
				fprintf(outfile2, " %4i %8s %12.0lf %14.6lf %10.2lf %12lf %8.4lf %7.0lf %7.0lf %10.2lf\n",
