...
* --------------------------------------------------------------------------- */
		...
		)
	{
		...
		sprintf_s(longstr1, "1 %5sU%8s  %02i%12.8lf  .00000000  00000-0 ",
		...
		sprintf_s(longstr2, "2 %5s %08.4lf%9.4lf %07.0lf %8.4lf %8.4lf %11.8lf 00000\n",
...
* --------------------------------------------------------------------------- */
		...
		)
	{
		...
		double rs[3], r3[3], v3[3], rteme[3], vteme[3], ritrf[3], vitrf[3], aitrf[3], ateme[3], rnom[3], vnom[3], 
			recinom[3], vecinom[3], aeci[3], recipert[3], vecipert[3], rpert[3], vpert[3];
		double dut1, tut1, jdut1, jdtt, jdftt, ttt, lod, xp, yp, ddpsi, ddeps, ddx, ddy, oldtn, jdxysstart,
		...
		for (i = firstob; i <= lastob; i++)
		{
			...
			if (currobsrec.obstype != 4)
			{
				...
				eOpt opt = e80;
				AstroLib::eci_ecef(ritrf, vitrf, aitrf, MathTimeLib::eFrom, recinom, vecinom, aeci, opt, iau80rec, iau06rec, jdtt, jdftt, 
					jdut1, jdxysstart, lod, xp, yp, ddpsi, ddeps, ddx, ddy, trans);
			} // if obstype
...
//	recinom[0], recinom[1], recinom[2], currobsrec.x, currobsrec.y, currobsrec.z);  // ritrf
			...
			for (j = 0; j < statesize; j++)
			{
				...
				if (currobsrec.obstype != 4)
				{
					...
					eOpt opt = e80;
					AstroLib::eci_ecef(ritrf, vitrf, aitrf, MathTimeLib::eFrom, r3, v3, aeci, opt, iau80rec, iau06rec, jdtt, jdftt,
						jdut1, lod, jdxysstart, xp, yp, ddpsi, ddeps, ddx, ddy, trans);
				} // if obstype
		...
		)
	{
			...
			!(sigmanew > sigmaold && sigmaold > sigmaold2 && sigmanew > 500000.0) && loops < 5)
    	{
			...
			findatwaatwb(firstob, lastob, statesize, percentchg, deltaamtchg,
				...
				derivType, cdam, cram, opt, jpldearr, jdjpldestart, iau80rec, iau06rec, eoparr,
